...
public class ExclusiveTimeOfFunctions {
    public static void main(String[] args) {
        ...
        for(int i : solution.exclusiveTime(2, logs)){
    ...
    class Solution {
        public int[] exclusiveTime(int n, List<String> logs) {
            ...
            for (String s : logs) {
                temp = s.split(":");
                process = new Process(Integer.parseInt(temp[0]), Integer.parseInt(temp[2]));
                if (temp[1].equals("start")) {
                    if (!stack.isEmpty()) {
                        old = stack.peek();
                        ints[old.id] += process.time - old.time;
                    }
                    stack.push(process);
                } else {
                    old = stack.pop();
                    ints[old.id] += process.time - old.time + 1;
                    if (!stack.isEmpty()) {
                        old = stack.pop();
                        old.time = process.time + 1;
                        stack.push(old);
                    }
                }
            }
