...
public class SimplifyPath{
    public static void main(String[] args) {
        ...
        System.out.println(solution.simplifyPath("/.hidden"));
    }
    ...
    public String simplifyPath(String path) {
        ...
        for(int i = 0; i < path.length(); i++){
            if(path.charAt(i) != '/'){
                ...
                if(path.charAt(i) == '.' && i + 1 < path.length() && path.charAt(i + 1) == '.'&& i + 2 == path.length()){
                    if(!stack.isEmpty()){
                        stack.pop();
                    }
                    i++;
                ...
                if(path.charAt(i) == '.' && i + 1 < path.length() && path.charAt(i + 1) == '.'&& i + 2 < path.length() && path.charAt(i + 2) == '/'){
                    if(!stack.isEmpty()){
                        stack.pop();
                    }
                    i++;
