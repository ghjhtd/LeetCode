...
class Solution {
    public String getHint(String secret, String guess) {
		...
		char[] data_guess = guess.toCharArray();
		...
		for(int i = 0; i < secret.length(); i++){
			secret_map.put(data[i], secret_map.getOrDefault(data[i], 0) + 1);
		}
		for(int i = 0; i < secret.length(); i++){
			guess_map.put(data_guess[i], guess_map.getOrDefault(data_guess[i], 0) + 1);
		}
		for(char key: secret_map.keySet()){
			if(guess_map.containsKey(key) && key != ' '){
				B += Math.min(secret_map.get(key), guess_map.get(key));
			}
		}
		return A + "A" + B + "B";
    }
